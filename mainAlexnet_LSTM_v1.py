...
if __name__=='__main__':
    ...
    model.add(td(layers.Conv2D(filters=64, kernel_size=(3, 3), strides=4, padding='same', activation=tf.nn.relu),
                 input_shape=(args.time_step, 64,64,3)))
    model.add(td(layers.MaxPool2D(pool_size=(3, 3), strides=2)))
    model.add(td(layers.Conv2D(filters=192, kernel_size=(3, 3), padding='same', activation=tf.nn.relu)))
    model.add(td(layers.MaxPool2D(pool_size=(3, 3), strides=2)))
    model.add(td(layers.Conv2D(filters=384, kernel_size=(3, 3), padding='same', activation=tf.nn.relu)))
    model.add(td(layers.Conv2D(filters=256, kernel_size=(3, 3), padding='same', activation=tf.nn.relu)))
    model.add(td(layers.Conv2D(filters=256, kernel_size=(3, 3), padding='same', activation=tf.nn.relu)))
    model.add(td(layers.MaxPool2D(pool_size=(3, 3), strides=2)))
